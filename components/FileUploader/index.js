 ...
 * @LastEditTime: 2022-10-19 15:22:42
 * @LastEditors: TingGe
 * @Description: 通用上传文件插件, 可配合 antd 4x form的使用
 * @FilePath: /tgbk-s/components/FileUploader/index.js
 */
...
class FileUploader extends React.Component {
  ...
  constructor (props) {
    ...
    this.state = {
      mode: props.mode,
      accept: props.accept,
      customUploadAccpetErrorTips: props.customUploadAccpetErrorTips,
      multiple: props.multiple, // 是否多选，bool， 与crop冲突，开启裁剪后只能单选
      maxLength: props.maxLength, // 最多上传数量， number
      maxSize: props.maxSize, // 文件最大大小，number
      cropOption: props.cropOption, // 裁剪属性 obj， 与multiple冲突，开启裁剪只能单选
      fileList: setInitValue(props.value), // value的映射， arr
      uploadProps: props.uploadProps // 额外的Upload的props
      // crop 裁剪属性
      // 属性	   类型	   默认	  说明
    ...
    return (
      <div className="dragger-upload-box">
        <p className="ant-upload-drag-icon">
          {this.state.mode === 'file' ? <FileTwoTone /> : <PictureOutlined />}
        </p>
        <p className="ant-upload-text">单击或拖动文件到此区域以上传</p>
        <p className="ant-upload-hint">
          {this.state.accept === '*'
            ? null
            : `只能上传限${this.state.accept}格式的文件，${getSize(
              this.state.maxSize
            )}以下！`}
        </p>
      </div>
  ...
  // 校验文件类型 返回 布尔
  checkFileType = (file) => {
    const { accept, customUploadAccpetErrorTips } = this.state;
    if (accept && '*' !== accept) {
      let messageStr = '';
      const hasErrorTips =
  ...
  render () {
    const { fileList, maxSize, maxLength, mode, cropOption, multiple, uploadProps } = this.state;
    ...
    return (
      <div className={styles['file-uploader-box']}>
        {mode === 'file' ? (
          <Upload.Dragger
            {...updateProps}
            name="files"
